 ...
 * variant. Each template value is checked as an unsigned value against its variant count before the
 * target table is indexed by the same mixed-radix template index used by threaded dispatch.
 */
...
public final class BytecodeHandlerMainDispatchAddressNode extends FixedWithNextNode implements Lowerable {
     ...
     * @param bytecodeHandlerTargets initialized target method table indexed by flattened template
     *            index
     */
    ...
    private ValueNode createTemplateIndex(StructuredGraph graph) {
        ...
        for (int i = 0; i < templateValues.size(); i++) {
            ...
            GraalError.guarantee(variants > 0, "Template variant count must be positive");
            ValueNode templateValue = templateValues.get(i);
            LogicNode inRange = graph.addOrUnique(IntegerBelowNode.create(templateValue, ConstantNode.forInt(variants, graph), NodeView.DEFAULT));
            graph.addBeforeFixed(this, graph.add(new FixedGuardNode(inRange, DeoptimizationReason.BoundsCheckException,
